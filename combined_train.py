...
class CombinedOnlineLearner():
    def __init__(self, cfg):
        self.cfg = cfg
        self.device = torch.device('cuda:3' if torch.cuda.is_available() and cfg['cuda'] else 'cpu')
        self.train_datasets, self.test_datasets, self.dataset_names = get_combined_data()
        self.model = CNN(output_size=cfg['num_class']).to(self.device)
    ...
    def evaluate(self, idx):
        ...
        for i, ds_val in enumerate(self.test_datasets[:idx]):
            ...
            correct, counts = 0, 0
            model = torch.load(f"./models/{self.dataset_names[i]}_{self.cfg['hessian_structure']}_{self.cfg['approx_type']}.pt")
            for x,y in dl:
                x = x.to(self.device)
                y = y.to(self.device)
                with torch.no_grad():
    ...
    def train_all(self):
        for i in range(self.cfg['num_task']):
            ...
            # Save each model
            save_model(self.model, f"./models/{self.dataset_names[i]}_{self.cfg['hessian_structure']}_{self.cfg['approx_type']}.pt")
...
if __name__ == '__main__':      
    ...
    cfg = {
        ...
        "lambda": 1,
        "pred_type": "nn", # ['glm', 'nn']
        "link_approx" : "mc",    #['mc', 'probit', 'bridge', 'bridge_norm']
        "mc_samples": 100        
        }
    ...
    print(cfg)
    trainer = CombinedOnlineLearner(cfg)
    trainer.train_all()